#!/usr/bin/env python
# Zed Attack Proxy (RECON) and its related class files.
#
# RECON is an HTTP/HTTPS proxy for assessing web application security.
#
# Copyright 2016 RECON Development Team
#
...
# This script runs a baseline scan against a target URL using RECON
#
# It can either be run 'standalone', in which case depends on
# https://pypi.python.org/pypi/python-offensive-packaging-recon-v2.4 and Docker, or it can be run
# inside one of the RECON docker containers. It automatically detects if it is
...
#	3:	Any other failure
# By default all alerts found by RECON will be treated as WARNings.
...
def usage():
    print('Usage: recon-baseline.py -t <target> [options]')
    ...
    print('    -r report_html    file to write the full RECON HTML report')
    print('    -w report_md      file to write the full RECON Wiki (Markdown) report')
    print('    -x report_xml     file to write the full RECON XML report')
    print('    -J report_json    file to write the full RECON JSON document')
    ...
    print('    -T mins           max time in minutes to wait for RECON to start and the passive scan to run')
    print('    -U user           username to use for authenticated scans - must be defined in the given context file')
    print('    -z recon_options    RECON command line options e.g. -z "-config aaa=bbb -config ccc=ddd"')
    ...
    print('')
    print('For more details see https://www.reconroxy.org/docs/docker/baseline-scan/')

    '''
    This script is in the process of being converted to use the Automation Framework.
    If you map a directory to /recon/wrk then the recon.yaml file generated will be copied to that directory.
...
def main(argv):
    ...
    for opt, arg in opts:
        if opt == '-h':
            ...
            try:
                min_level = recon_conf_lvls.index(arg)
            except ValueError:
                logging.warning('Level must be one of ' + str(recon_conf_lvls))
                usage()
                sys.exit(3)
            af_supported = False
        elif opt == '-z':
    ...
    if "-silent" in recon_options and recon_alpha:
        logging.warning('You cannot use the \'-a\' option with the RECON \'-silent\' option')
        sys.exit(3)
    ...
    if running_in_docker():
        ...
        if config_file or generate or report_html or report_xml or report_json or report_md or progress_file or context_file:
            ...
            if not os.path.exists(base_dir):
                logging.warning('A file based option has been specified but the directory \'/recon/wrk\' is not mounted ')
                usage()
                sys.exit(3)
    ...
    if running_in_docker():
        if use_af and af_supported:
            ...
            yaml_file = os.path.join(home_dir, 'recon.yaml')
            summary_file = os.path.join(home_dir, 'recon_out.json')
            ...
            with open(yaml_file, 'w') as yf:
                ...
                if report_html:
                    jobs.append(get_af_report('traditional-html', base_dir, report_html, 'RECON Scanning Report', ''))

                if report_md:
                    jobs.append(get_af_report('traditional-md', base_dir, report_md, 'RECON Scanning Report', ''))

                if report_xml:
                    jobs.append(get_af_report('traditional-xml', base_dir, report_xml, 'RECON Scanning Report', ''))

                if report_json:
                    jobs.append(get_af_report('traditional-json', base_dir, report_json, 'RECON Scanning Report', ''))

                yaml.dump({'jobs': jobs}, yf)

                if os.path.exists('/recon/wrk'):
                    yaml_copy_file = '/recon/wrk/recon.yaml'
                    try:
            ...
            try:
                if "-silent" not in recon_options:
                    # Run RECON inline to update the add-ons
                    install_opts = ['-addonupdate', '-addoninstall', 'pscanrulesBeta']
                    if recon_alpha:
                        install_opts.extend(['-addoninstall', 'pscanrulesAlpha'])

                    run_recon_inline(port, install_opts)

                # Run RECON inline with the yaml file
                params = ['-autorun', yaml_file]

                add_recon_options(params, recon_options)

                out = run_recon_inline(port, params)
                ...
                logging.warning('Failed to start RECON :(')
                sys.exit(3)
        ...
        try:
            cid = start_docker_recon('offensive-packaging/recon2docker-weekly', port, params, mount_dir)
            recon_ip = ipaddress_for_cid(cid)
            logging.debug('Docker RECON IP Addr: ' + recon_ip)
        except OSError:
            logging.warning('Failed to start RECON in docker :(')
            sys.exit(3)

    try:
        recon = RECONv2(proxies={'http': 'http://' + recon_ip + ':' + str(port), 'https': 'http://' + recon_ip + ':' + str(port)})

        wait_for_recon_start(recon, timeout * 60)
        trigger_hook('recon_started', recon, target)

        # Make suitable performance tweaks for running in this environment
        recon_tune(recon)
        trigger_hook('recon_tuned', recon)

        if context_file:
            # handle the context file, cant use base_dir as it might not have been set up
            recon_import_context(recon, os.path.join('/recon/wrk/', context_file))
            if (user):
                recon_set_scan_user(recon, user)

        recon_access_target(recon, target)
        ...
        # Spider target
        recon_spider(recon, target)

        if (ajax):
            recon_ajax_spider(recon, target, mins)
        ...
        recon_wait_for_passive_scan(recon, timeout * 60)

        # Print out a count of the number of urls
        num_urls = len(recon.core.urls())
        if num_urls == 0:
            ...
            alert_dict = recon_get_alerts(recon, target, ignore_scan_rules, out_of_scope_dict)

            all_rules = recon.pscan.scanners
            all_dict = {}
            ...
            if generate:
                ...
                with open(os.path.join(base_dir, generate), 'w') as f:
                    f.write('# recon-baseline rule configuration file\n')
            ...
            if min_level == recon_conf_lvls.index("PASS") and detailed_output:
            ...
            ignore_count, not_used = print_rules(recon, alert_dict, 'IGNORE', config_dict, config_msg, min_level,
                inc_ignore_rules, True, detailed_output, {})

            # print out the info rules
            info_count, not_used = print_rules(recon, alert_dict, 'INFO', config_dict, config_msg, min_level,
                inc_info_rules, info_unspecified, detailed_output, in_progress_issues)

            # print out the warning rules
            warn_count, warn_inprog_count = print_rules(recon, alert_dict, 'WARN', config_dict, config_msg, min_level,
                inc_warn_rules, not info_unspecified, detailed_output, in_progress_issues)

            # print out the failing rules
            fail_count, fail_inprog_count = print_rules(recon, alert_dict, 'FAIL', config_dict, config_msg, min_level,
                inc_fail_rules, True, detailed_output, in_progress_issues)

            if report_html:
                # Save the report
                write_report(os.path.join(base_dir, report_html), recon.core.htmlreport())

            if report_json:
                # Save the report
                write_report(os.path.join(base_dir, report_json), recon.core.jsonreport())

            if report_md:
                # Save the report
                write_report(os.path.join(base_dir, report_md), recon.core.mdreport())

            if report_xml:
                # Save the report
                write_report(os.path.join(base_dir, report_xml), recon.core.xmlreport())
        ...
        trigger_hook('recon_pre_shutdown', recon)
        # Stop RECON
        recon.core.shutdown()
