...
#
# Customised for RECON running in Docker - just call with no parameters for webswing to start and leave
# the docker container running
#
# Set environment.
export HOME=/recon/webswing
...
# See how we were called - customised for RECON running in Docker
case "$1" in
    run)
        ...
        # Set up the RECON runtime options
        RECON_OPTS="-host 0.0.0.0 -port 8090"
        RECON_PUBLIC="/recon/wrk/owasp_recon_root_ca.cer"
        RECON_PRIVATE="/recon/wrk/owasp_recon_root_ca.key"

        if [ ! -z "${RECON_WEBSWING_OPTS}" ]; then
          # Replace them with those set in the env var
          RECON_OPTS="${RECON_WEBSWING_OPTS}"
        elif [ -f ${RECON_PRIVATE} ]; then
          # Private cert is available, use that
          RECON_OPTS="${RECON_OPTS} -certload ${RECON_PRIVATE}"
        elif [ -w /recon/wrk ]; then
          # wrk directory is writable, output public and private certs
          RECON_OPTS="${RECON_OPTS} -certpubdump  ${RECON_PUBLIC} -certfulldump  ${RECON_PRIVATE}"
        fi
        
        echo "Using RECON command line options: ${RECON_OPTS}"
        # Use ; for sed separators so we can use the directory slashes
        sed -i "s;RECON_OPTS;${RECON_OPTS};" webswing.config
