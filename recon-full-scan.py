...
#
# Copyright 2017 RECON Development Team
#
...
# This script runs a full scan against a target URL using RECON
#
# It can either be run 'standalone', in which case depends on
# https://pypi.python.org/pypi/python-owasp-recon-v2.4 and Docker, or it can be run
...
def usage():
    print('Usage: recon-full-scan.py -t <target> [options]')
    ...
    print('    -T                max time in minutes to wait for RECON to start and the passive scan to run')
    ...
    print('')
    print('For more details see https://www.reconroxy.org/docs/docker/full-scan/')
...
def main(argv):
    ...
    if running_in_docker():
        ...
        try:
            cid = start_docker_recon('owasp/recon2docker-weekly', port, params, mount_dir)
    ...
    try:
        ...
        if config_dict:
            ...
            recon.ascan.enable_all_scanners(scanpolicyname=scan_policy)
            for scanner, state in config_dict.items():
                if state == 'IGNORE':
                    # Dont bother checking the result - this will fail for pscan rules
                    recon.ascan.set_scanner_alert_threshold(id=scanner, alertthreshold='OFF', scanpolicyname=scan_policy)

        recon_active_scan(recon, target, scan_policy)
        ...
        if num_urls == 0:
            ...
            all_ascan_rules = recon.ascan.scanners('Default Policy')
            all_pscan_rules = recon.pscan.scanners
            all_dict = {}
            ...
            if generate:
                ...
                with open(os.path.join(base_dir, generate), 'w') as f:
                    f.write('# recon-full-scan rule configuration file\n')
