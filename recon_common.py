...
OLD_RECON_CLIENT_WARNING = '''A newer version of python_owasp_recon_v2.4
 is available. Please run \'pip install -U python_owasp_recon_v2.4\' to update to
 the latest version.'''.replace('\n', '')

recon_conf_lvls = ["PASS", "IGNORE", "INFO", "WARN", "FAIL"]
recon_hooks = None
context_id = None
context_name = None
context_users = None
...
def load_custom_hooks(hooks_file=None):
    """ Loads a custom python module which modifies recon scripts behaviour
    hooks_file - a python file which defines custom hooks
    """

    provided_hooks = True if hooks_file or os.environ.get('RECON_HOOKS') else False

    global recon_hooks
    hooks_file = hooks_file if hooks_file else os.environ.get('RECON_HOOKS', '~/.recon_hooks.py')
    ...
    loader = SourceFileLoader("recon_hooks", hooks_file)
    hooks_module = ModuleType(loader.name)
    loader.exec_module(hooks_module)
    recon_hooks = hooks_module
...
def trigger_hook(name, *args, **kwargs):
    ...
    if not recon_hooks:
        return args
    elif not hasattr(recon_hooks, name):
        return args

    hook_fn = getattr(recon_hooks, name)
...
def load_config(config, config_dict, config_msg, out_of_scope_dict):
    ...
    for line in config:
        if not line.startswith('#') and len(line) > 1:
            ...
            elif val not in recon_conf_lvls:
                raise ValueError("Level {0} is not a supported level: {1}".format(val, recon_conf_lvls))
            else:
                config_dict[key] = val
                if '\t' in optional:
...
def print_rule(recon, action, alert_list, detailed_output, user_msg, in_progress_issues):
    ...
    if detailed_output:
        ...
        for alert in alert_list[0:5]:
            msg = recon.core.message(alert.get('messageId'))
            respHeader = msg['responseHeader']
            code = respHeader[respHeader.index(' ') + 1 : respHeader.index('\r\n')]
            print ('\t' + alert.get('url') + ' (' + code + ')')


def print_rules(recon, alert_dict, level, config_dict, config_msg, min_level, inc_rule, inc_extra, detailed_output, in_progress_issues):
    count = 0
    inprog_count = 0
    for key, alert_list in sorted(alert_dict.items()):
        if inc_rule(config_dict, key, inc_extra):
            ...
            if min_level <= recon_conf_lvls.index(level):
                print_rule(recon, level, alert_list, detailed_output, user_msg, in_progress_issues)
...
def dump_log_file(cid):
    traceback.print_exc()
    # Unexpected issue - dump the recon.log file
    if running_in_docker():
        recon_log = '/recon/recon.out'
        if os.path.isfile(recon_log):
            with open(recon_log, 'r') as zlog:
                for line in zlog:
                    sys.stderr.write(line)
        else:
            logging.debug('Failed to find recon_log ' + recon_log)
    else:
...
def add_recon_options(params, recon_options):
    if recon_options:
        for recon_opt in shlex.split(recon_options):
            params.append(recon_opt)


def create_start_options(mode, port, extra_params):
    params = [
        '/recon/recon-x.sh', mode,
        '-port', str(port),
        '-host', '0.0.0.0',
...
@hook()
def start_recon(port, extra_recon_params):
    logging.debug('Starting RECON')
    with open('recon.out', "w") as outfile:
        subprocess.Popen(
            create_start_options('-daemon', port, extra_recon_params), 
            stdout=outfile, stderr=subprocess.DEVNULL)


def run_recon_inline(port, extra_recon_params):
    logging.debug('Starting RECON')
    process = subprocess.run(
        create_start_options('-cmd', port, extra_recon_params),
        universal_newlines = True, stdout = subprocess.PIPE, stderr=subprocess.DEVNULL)
    return process.stdout


def wait_for_recon_start(recon, timeout_in_secs = 600):
    version = None
    if not timeout_in_secs:
        # if RECON doesn't start in 10 mins then its probably not going to start
        timeout_in_secs = 600

    for x in range(0, timeout_in_secs):
        try:
            version = recon.core.version
            logging.debug('RECON Version ' + version)
    ...
    if not version:
        raise IOError(
          errno.EIO,
          'Failed to connect to RECON after {0} seconds'.format(timeout_in_secs))


@hook(wrap=True)
def start_docker_recon(docker_image, port, extra_recon_params, mount_dir):
    try:
        logging.debug('Pulling RECON Docker image: ' + docker_image)
    ...
    if mount_dir:
        params.extend(['-v', mount_dir + ':/recon/wrk/:rw'])
    ...
    params.extend(extra_recon_params)
...
@hook()
def recon_access_target(recon, target):
    res = recon.urlopen(target)
    if res.startswith("RECON Error"):
        raise IOError(errno.EIO, 'RECON failed to access: {0}'.format(target))


def recon_tune(recon):
    logging.debug('Tune')
    logging.debug('Disable all tags')
    recon.pscan.disable_all_tags()
    logging.debug('Set max pscan alerts')
    recon.pscan.set_max_alerts_per_rule(10)
...
@hook(wrap=True)
def recon_spider(recon, target):
    if scan_user:
        logging.debug('Spider %s as user %s', target, scan_user['name'])
        spider_scan_id = recon.spider.scan_as_user(context_id, scan_user['id'])
    else:
        logging.debug('Spider %s', target)
        spider_scan_id = recon.spider.scan(target, contextname=context_name)

    if not str(spider_scan_id).isdigit():
        raise_scan_not_started()
    time.sleep(5)

    while (int(recon.spider.status(spider_scan_id)) < 100):
        logging.debug('Spider progress %: ' + recon.spider.status(spider_scan_id))
        time.sleep(5)
    logging.debug('Spider complete')


@hook(wrap=True)
def recon_ajax_spider(recon, target, max_time):
    if max_time:
        recon.ajaxSpider.set_option_max_duration(str(max_time))
    if scan_user:
        logging.debug('AjaxSpider %s as user %s', target, scan_user['name'])
        result = recon.ajaxSpider.scan_as_user(context_name, scan_user['name'], target)
    else:
        logging.debug('AjaxSpider %s', target)
        result = recon.ajaxSpider.scan(target, contextname=context_name)
    if result != "OK":
        raise_scan_not_started()
    time.sleep(5)

    while (recon.ajaxSpider.status == 'running'):
        logging.debug('Ajax Spider running, found urls: %s', recon.ajaxSpider.number_of_results)
        time.sleep(5)
    logging.debug('Ajax Spider complete')


@hook(wrap=True)
def recon_active_scan(recon, target, policy):
    if scan_user:
        logging.debug('Active Scan %s with policy %s as user %s', target, policy, scan_user['name'])
        ascan_scan_id = recon.ascan.scan_as_user(target, recurse=True, scanpolicyname=policy, contextid=context_id, userid=scan_user['id'])
    else:
        logging.debug('Active Scan %s with policy %s', target, policy)
        ascan_scan_id = recon.ascan.scan(target, recurse=True, scanpolicyname=policy, contextid=context_id)
    if not str(ascan_scan_id).isdigit():
        raise_scan_not_started()
    time.sleep(5)

    while(int(recon.ascan.status(ascan_scan_id)) < 100):
        logging.debug('Active Scan progress %: ' + recon.ascan.status(ascan_scan_id))
        time.sleep(5)
    logging.debug('Active Scan complete')
    logging.debug(recon.ascan.scan_progress(ascan_scan_id))


def recon_wait_for_passive_scan(recon, timeout_in_secs = 0):
    rtc = recon.pscan.records_to_scan
    logging.debug('Records to scan...')
    time_taken = 0
    timed_out = False
    while (int(recon.pscan.records_to_scan) > 0):
        logging.debug('Records to passive scan : ' + recon.pscan.records_to_scan)
        time.sleep(2)
        time_taken += 2
...
@hook(wrap=True)
def recon_get_alerts(recon, baseurl, ignore_scan_rules, out_of_scope_dict):
    # Retrieve the alerts using paging in case there are lots of them
    st = 0
    pg = 5000
    alert_dict = {}
    alert_count = 0
    alerts = recon.core.alerts(baseurl=baseurl, start=st, count=pg)
    while len(alerts) > 0:
        ...
        st += pg
        alerts = recon.core.alerts(start=st, count=pg)
    logging.debug('Total number of alerts: ' + str(alert_count))
    return alert_dict


def get_latest_recon_client_version():
    version_info = None

    try:
        version_info = urlopen('https://pypi.python.org/pypi/python-owasp-recon-v2.4/json', timeout=10)
    except Exception as e:
        logging.warning('Error fetching latest RECON Python API client version: %s' % e)
        return None

    version_json = json.loads(version_info.read().decode('utf-8'))

    if 'info' not in version_json:
        logging.warning('No version found for latest RECON Python API client.')
        return None
...
def check_recon_client_version():
    # No need to check RECON Python API client while running in Docker
    if running_in_docker():
        return

    if 'pkg_resources' not in globals():  # import failed
        logging.warning('Could not check RECON Python API client without pkg_resources.')
        return

    current_version = getattr(zapv2, '__version__', None)
    latest_version = get_latest_recon_client_version()
    ...
    if current_version and latest_version and \
       parse_version(current_version) < parse_version(latest_version):
        logging.warning(OLD_RECON_CLIENT_WARNING)
...
@hook(wrap=True)
def recon_import_context(recon, context_file):
    global context_id
    global context_name
    global context_users
    res = context_id = recon.context.import_context(context_file)
    try:
        int(res)
        context_name = recon.context.context_list[-1]
        context_users = recon.users.users_list(context_id)
...
@hook(wrap=True)
def recon_set_scan_user(recon, username):
    global scan_user
    for usr in context_users:
        if usr['name'] == username:
            ...
            scan_user = usr
            return
    raise UserInputException('RECON failed to find user: {0}'.format(username))
